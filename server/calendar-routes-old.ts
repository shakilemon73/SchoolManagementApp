...
export function registerCalendarRoutes(app: Express) {
  ...
  app.get('/api/calendar/events', async (req: Request, res: Response) => {
    try {
      ...
      if (eventType) {
        whereConditions.push(eq(calendarEvents.type, eventType as string));
      }
  ...
  app.put('/api/calendar/events/:id', async (req: Request, res: Response) => {
    try {
      const eventId = parseInt(req.params.id);
      const validatedData = calendarEventsInsertSchema.partial().parse(req.body);
  ...
  // Get events for specific date range (dashboard widget)
