...
export function registerIdCardTableCreationRoute(app: Express) {
  app.post("/api/admin/create-id-card-tables", async (req: Request, res: Response) => {
    try {
      console.log('Creating ID card tables...');
      ...
      res.json({
        success: true,
        message: 'ID card tables created successfully with sample data'
      });

    } catch (error) {
      console.error('Error creating ID card tables:', error);
      res.status(500).json({
        success: false,
        message: 'Failed to create ID card tables',
